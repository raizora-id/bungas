...
// import { CategoryBar } from "@ui/components/charts/category-bar"
    ...
    <section aria-labelledby="transaction-policy-heading">
      <div className="grid grid-cols-1 gap-x-14 gap-y-8 md:grid-cols-3">
        ...
        <div className="md:col-span-2">
          ...
          >
            {blacklist.map((item) => (
              <li key={item.category} className="flex items-start gap-2.5">
                <span
                  className={cn(item.color, "mt-[2px] size-2.5 rounded-sm")}
                  aria-hidden="true"
                />
                <div>
          ...
          <Button
            variant="secondary"
            className={cn("mt-4 w-full sm:w-fit", isKeyword && "hidden")}
            onClick={(e) => {
              e.preventDefault()
              setIsKeyword(!isKeyword)
            }}
